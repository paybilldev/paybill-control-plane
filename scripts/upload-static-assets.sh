...
# If asset CDN is specifically disabled (i.e. platform self-hosted), we skip
if [[ "$FORCE_ASSET_CDN" == "-1" ]]; then
    echo "Skipping asset upload. Set FORCE_ASSET_CDN=1 to execute."
    exit 0
fi
...
aws s3 sync "$STATIC_DIR" "s3://$BUCKET_NAME/$SITE_NAME/${GIT_COMMIT_SHA:0:12}/_next/static" \
...
aws s3 sync "$PUBLIC_DIR" "s3://$BUCKET_NAME/$SITE_NAME/${GIT_COMMIT_SHA:0:12}" \
    --endpoint-url "$ASSET_CDN_S3_ENDPOINT" \
    --cache-control "public,max-age=604800,immutable" \
    --region auto \
    --only-show-errors
