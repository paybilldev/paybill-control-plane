...
export const AUTH_NAVIGATOR_LOCK_DISABLED_KEY =
  process.env.NEXT_PUBLIC_AUTH_NAVIGATOR_LOCK_KEY || 'paybill.dashboard.auth.navigatorLock.disabled'
...
const debug = safeGetLocalStorage(AUTH_DEBUG_KEY) === 'true'

const persistedDebug = safeGetLocalStorage(AUTH_DEBUG_PERSISTED_KEY) === 'true'
...
export const internals = {
  ...
  debug: !!(
    ...
    globalThis.localStorage.getItem('paybill.auth.locks.debug') === 'true'
  ),
}

const DEFAULT_OPTIONS: Omit<Required<Options>, 'fetch' | 'storage' | 'userStorage' | 'lock'> = {
  url: API_URL,
 ...
 * especially useful for shared-memory execution environments such as
 * Fluid Compute, AWS Lambda or Edge Functions. Regardless of how
...
export default class ApiClient {
  ...
  constructor(options: Options) {
    ...
    this.instanceID = ApiClient.nextInstanceID[this.storageKey] ?? 0
    ApiClient.nextInstanceID[this.storageKey] = this.instanceID + 1
    ...
    if (this.instanceID > 0 && isBrowser) {
      const message = `${this._logPrefix()} Multiple ApiClient instances detected in the same browser context. It is not an error, but this should be avoided as it may produce undefined behavior when used concurrently under the same storage key.`
  ...
  private _debug(...args: any[]): ApiClient {
    if (this.logDebugMessages) {
    ...
    try {
      const res = await _request(this.fetch, 'POST', `${this.url}/signup`, {
        ...
        body: {
          ...
          meta_security: { captcha_token: credentials?.options?.captchaToken },
        },
      ...
      if ('email' in credentials) {
        ...
        res = await _request(this.fetch, 'POST', `${this.url}/signup`, {
          ...
          body: {
            ...
            password,
            data: options?.data ?? {},
            meta_security: { captcha_token: options?.captchaToken },
  ...
  async verifyOtp(params: VerifyOtpParams): Promise<AuthResponse> {
    try {
      ...
      const { data, error } = await _request(this.fetch, 'POST', `${this.url}/verify`, {
        ...
        body: {
          ...params,
          meta_security: { captcha_token: captchaToken },
        },
        redirectTo,
        xform: _sessionResponse,
      })
      ...
      const result = await _request(this.fetch, 'POST', `${this.url}/sso`, {
        body: {
          ...
          ...(params?.options?.captchaToken
            ? { meta_security: { captcha_token: params.options.captchaToken } }
            : null),
   ...
   * performs a network request to the Auth server, so the returned
  ...
  > {
    try {
      ...
      if (actuallyExpiresIn * 1000 <= AUTO_REFRESH_TICK_DURATION_MS) {
        console.warn(
          `api: Session as retrieved from URL expires in ${actuallyExpiresIn}s, should have been closer to ${expiresIn}s`
        )
      }
      ...
      if (timeNow - issuedAt >= 120) {
        console.warn(
          'api: Session as retrieved from URL was issued over 120s ago, URL could be stale',
          issuedAt,
          expiresAt,
          timeNow
        )
        ...
        console.warn(
          'api: Session as retrieved from URL was issued in the future? Check the device clock for skew',
          issuedAt,
          expiresAt,
          timeNow
        )
   ...
   */
  async signOut(options: SignOut = { scope: 'global' }): Promise<{ error: Error | null }> {
    await this.initializePromise
  ...
  async __signOut(
    ...
    if (SIGN_OUT_SCOPES.indexOf(scope) < 0) {
      throw new Error(`auth: Parameter scope must be one of ${SIGN_OUT_SCOPES.join(', ')}`)
    }
  ...
  protected async _signOut(
    ...
    return await this._useSession(async (result) => {
      ...
      if (accessToken) {
        const { error } = await this.__signOut(accessToken, scope)
        if (error) {
   ...
   * so calling other Client APIs that also try to acquire the
   ...
   * necessary.
   *
   * If you set the {@link Options#autoRefreshToken} you don't need
...
export async function navigatorLock<R>(
  ...
  if (internals.debug) {
    console.log('@paybill/auth: navigatorLock: acquire lock', name, acquireTimeout)
  }
  ...
  if (acquireTimeout > 0) {
    setTimeout(() => {
      ...
      if (internals.debug) {
        console.log('@paybill/auth: navigatorLock acquire timed out', name)
      }
    }, acquireTimeout)
  }
  ...
  return await Promise.resolve().then(() =>
    globalThis.navigator.locks.request(
      ...
      async (lock) => {
        if (lock) {
          if (internals.debug) {
            console.log('@paybill/auth: navigatorLock: acquired', name, lock.name)
          }
          ...
          try {
            ...
            if (internals.debug) {
              console.log('@paybill/auth: navigatorLock: released', name, lock.name)
            }
          }
        } else {
          if (acquireTimeout === 0) {
            if (internals.debug) {
              console.log('@paybill/auth: navigatorLock: not immediately available', name)
            }
            ...
            if (internals.debug) {
              try {
                ...
                console.log(
                  '@paybill/auth: Navigator LockManager state',
                  JSON.stringify(result, null, '  ')
                )
              } catch (e: any) {
                console.warn('@paybill/auth: Error when querying Navigator LockManager state', e)
              }
            }
            ...
            console.warn(
              '@paybill/auth: Navigator LockManager returned a null lock when using #request without ifAvailable set to true, it appears this browser is not following the LockManager spec https://developer.mozilla.org/en-US/docs/Web/API/LockManager/request'
            )
