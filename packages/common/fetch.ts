...
import { API_VERSION_HEADER_NAME, API_VERSIONS } from './constants'
import {
...
import { expiresAt, parseResponseAPIVersion } from './helpers'
...
export async function handleError(error: unknown) {
  ...
  if (
    responseAPIVersion &&
    responseAPIVersion.getTime() >= API_VERSIONS['2026-01-01'].timestamp &&
    typeof data === 'object' &&
    data &&
...
interface ApiClientRequestOptions extends FetchOptions {
  jwt?: string
  redirectTo?: string
  body?: object
  query?: { [key: string]: string }
  /**
   * Function that transforms api response from api into a desirable / standardised format
   */
  xform?: (data: any) => any
}
...
export async function _request(
  ...
  if (!headers[API_VERSION_HEADER_NAME]) {
    headers[API_VERSION_HEADER_NAME] = API_VERSIONS['2026-01-01'].name
  }
