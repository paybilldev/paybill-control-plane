...
export async function getFeatureFlags(API_URL: string) {
  try {
    ...
    return data as TelemetryCallFeatureFlagsResponse
  } catch (error: any) {
...
export async function trackFeatureFlag(API_URL: string, body: TelemetryFeatureFlagBody) {
...
export type FeatureFlagContextType = {
  ...
  posthog: TelemetryCallFeatureFlagsResponse
  hasLoaded?: boolean
}
    ...
    async function processFlags() {
      ...
      if (flagValues.length > 0) {
        ...
        try {
          ...
          overridesCookieValue = JSON.parse(cookies['flag-overrides'])
        } catch {}
