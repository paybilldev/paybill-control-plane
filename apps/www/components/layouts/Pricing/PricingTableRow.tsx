import { InfoTooltip, FeatureIcon } from 'ui'
import { Fragment, ReactNode } from 'react'
import { IconPricingIncludedCheck, IconPricingMinus } from './PricingIcons'
import { FeatureKey } from 'data/pricing'
...
export const pricingTooltips: PricingTooltips = {
  'database.dedicatedDatabase': {
    main: 'A Postgres or MySQL database with no restrictions? You get it. No pseudo limited users, you are the root user.  No caveats.',
  },
  'database.size': {
  ...
  'database.advancedDiskConfig': {
    main: 'Paybill databases are backed by high performance SSD disks. The disk can be scaled up to 60 TB, 80,000 IOPS and 4,000 MB/s throughput.',
  },
  ...
  'database.bandwidth': {
    main: 'Billing is based on the total sum of all outgoing traffic (includes Database, Storage, Auth, API, Supavisor, Log Drains) in GB throughout your billing period.',
  },
  'auth.totalUsers': {
  ...
  'security.accessRoles': {
    main: (
      <span className="prose text-xs">
        Paybill provides granular access controls to manage permissions across your organizations
      ...
      >
        ...
        >
          <div className="flex items-center gap-4">
            {props.icon && <FeatureIcon icon={props.icon} color="green" />}
  ...
  return (
    <table className="mt-8 w-full -scroll-mt-5" id={`${props.sectionId}-mobile`}>
      <caption className="bg-background border-default border-t px-4 py-3 text-left text-sm font-medium text-foreground">
        <span className="flex items-center gap-2">
          {category.icon ? <FeatureIcon icon={props.icon} /> : null}
