...
import { ChevronDown, Search, X as CloseIcon, AlignJustify, Grid } from 'lucide-react'
...
function EventFilters({ allEvents, setEvents, categories, onDemandEvents, view, setView }: Props) {
  ...
  const handleViewSelection = () => {
    setView((prevView: 'list' | 'grid') => {
      ...
      localStorage.setItem('paybill-event-view', newValue)
  ...
  const handleSearchByText = (text: string) => {
    ...
    router.replace(`/events?q=${text}`, undefined, {
      shallow: true,
      scroll: false,
    })
