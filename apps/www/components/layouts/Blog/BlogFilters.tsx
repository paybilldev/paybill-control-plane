...
import type { BlogView, PostTypes } from '~/types'
...
function BlogFilters({ allPosts, setPosts, view, setView }: Props) {
  ...
  const allCategories = ['all', 'product', 'company', 'database', 'developers', 'engineering']
  ...
  const handleSearchByText = (text: string) => {
    ...
    router.replace(`/blog?q=${text}`, undefined, {
      shallow: true,
      scroll: false,
    })
  ...
  const handleViewSelection = () => {
    setView((prevView: 'list' | 'grid') => {
      ...
      localStorage.setItem('paybill-blog-view', newValue)
