...
# If asset CDN is specifically disabled (i.e. self-hosted), we skip
if [[ "$FORCE_ASSET_CDN" == "false" ]]; then
    echo "Skipping asset upload. Set FORCE_ASSET_CDN="true" or NODE_ENV=production to execute."
    exit 0
fi

# Check for force env var or production environment
if [[ "$FORCE_ASSET_CDN" != "true" ]] && [[ "$NODE_ENV" != "production" ]]; then
    echo "Skipping asset upload. Set FORCE_ASSET_CDN='true' or NODE_ENV=production to execute."
    exit 0
fi
...
aws s3 sync "$STATIC_DIR" "s3://$BUCKET_NAME/$SITE_NAME/_next/static" \
...
aws s3 sync "$PUBLIC_DIR" "s3://$BUCKET_NAME/$SITE_NAME" \
    --endpoint-url "$ASSET_CDN_S3_ENDPOINT" \
    --cache-control "public,max-age=604800,immutable" \
    --region auto \
    --only-show-errors
