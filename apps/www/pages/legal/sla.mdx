...
export const meta = {
  title: 'Service Level Agreement | Paybill',
  description: 'Paybill Service Level Agreement',
}
...
The following Service Level Agreement, which is incorporated into and forms part of the Subscription Agreement between Welgon Limited. ("Paybill") and Customer (the "Agreement"), will apply to the Services for Enterprise Customers specified in an Order Form during the applicable Subscription Term.
...
For the current release stage of each Paybill product and feature see [Paybill features](`${process.env.PLATFORM_PUBLIC_URL}`/features).
...
- Email sending issues when using the default (provisional) configuration [not intended for production use](/docs/guides/auth/auth-smtp).
...
Any period during which Paybill is unavailable for permitted users to manage projects, view logs, or interact with platform resources globally.
