...
function FeaturesPage() {
  ...
  const HAS_ACTIVE_FILTERS = selectedFeatures.length || searchTerm.length

  const features = Array.from(new Set(featureData.flatMap((feature) => feature.features)))
  ...
  }, [searchTerm, selectedFeatures, updateQueryParamsDebounced])

  // Sync state with query parameters when they change
  useEffect(() => {
    if (query.q !== searchTerm) setSearchTerm((query.q as string) || '')
    if (query.features !== selectedFeatures.join(',')) {
      setSelectedFeatures((query.features as string)?.split(',') || [])
    }
  }, [query.q, query.features])
  ...
  // Filter features based on search term and selected features
  const filteredFeatures = featureData.filter((feature) => {
    ...
    const matchesFeature =
      ...
      feature.features.some((feature) => selectedFeatures.includes(feature))

    return matchesSearch && matchesFeature
  })

  const meta = {
    title: 'Paybill Features',
    description:
      ...
      <DefaultLayout>
        <SectionContainer className="!py-0 sm:!px-0">
          <div className="border border-muted rounded-xl bg-alternative my-4 px-6 py-8 md:py-10 lg:px-16 lg:py-20 xl:px-20 bg-center bg-cover bg-[url('/images/features/features-cover-light.svg')] dark:bg-[url('/images/features/features-cover-dark.svg')]">
            ...
            >
              <h1 className="h1 text-foreground !m-0">Paybill Features</h1>
