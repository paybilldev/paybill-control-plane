...
setup('Global Setup', async ({ page }) => {
  ...
  await page.goto(platformUrl).catch((err) => {
    console.error(
      `\n 🚨 Setup Error 
Platform is not available at: ${platformUrl}

Please ensure:
  1. Platform is running in the expected URL
...
`
  ...
  console.log(`\n ✅ Platform is running at ${platformUrl}`)
...
To start API locally, run:
  ...
  const signInUrl = `${platformUrl}/sign-in`
